...
public class PlayerStatsDatabase {
    ...
    public void editRecord(PlayerStatsModel r) throws SQLException {
        try (Connection conn = getConnection()) {
            try (PreparedStatement statement = conn.prepareStatement("UPDATE stats SET kills = ?, deaths = ?, killstreak = ? WHERE uuid = ?")) {
    ...
    public PlayerStatsModel readRecord(UUID uuid) throws SQLException {
        try (Connection conn = getConnection()) {
            try (PreparedStatement statement = conn.prepareStatement("SELECT * FROM stats WHERE uuid = ?")) {
            ...
            try (PreparedStatement statement = conn.prepareStatement("INSERT INTO stats(uuid, kills, deaths, killstreak) VALUES (?, ?, ?, ?)")) {
                statement.setString(1, uuid.toString());
